...
public class CorridorNode : Node
{
    ...
    private void ProcessRoomInRelationUpOrDown(Node structure1, Node structure2)
    {
        ...
        {
            topStucture = structure2;
        }
        else
        {
            topStucture = possibleNeighboursInTopStructure[0];
        }
        int x = GetValidXForNeighborUpDown(
                bottomStructure.TopLeftAreaCorner,
                bottomStructure.TopRightAreaCorner,
                topStucture.BottomLeftAreaCorner,
                topStucture.BottomRightAreaCorner);
        while(x == -1 && sortedBottomStructure.Count > 1)
        {
            sortedBottomStructure = sortedBottomStructure.Where(child => child.TopLeftAreaCorner.x != topStucture.TopLeftAreaCorner.x).ToList();
        ...
        TopRightAreaCorner = new Vector2Int(x + this.corridorWidth, topStucture.BottomLeftAreaCorner.y);
    }

    private int GetValidXForNeighborUpDown(Vector2Int bottomNodeLeft,
